...
class setupNLP:
    ...
    def __init__(self, horizon, Ts, Q, P, R, params, model, track, track_cons=False,
                 terminal_cost_func=None, terminal_set=None, ey_weight=0.0):
        ...
        if terminal_cost_func is not None:
            terminal_cost = terminal_cost_func(x[:2, -1])
        else:
            terminal_cost = (x[:xref_size,-1]-xref[:xref_size,-1]).T @ P @ (x[:xref_size,-1]-xref[:xref_size,-1])

        cost_tracking += terminal_cost
        constraints = x[:,0] - x0
        ...
        for idh in range(horizon):
            ...
            if ey_weight > 0:
                dx_ey = x[0, idh+1] - xref[0, idh+1]
                dy_ey = x[1, idh+1] - xref[1, idh+1]
                cost_tracking += ey_weight * (dx_ey*dx_ey + dy_ey*dy_ey)
        ...
        if terminal_set is not None:
            A_hull, b_hull = terminal_set
            for i in range(A_hull.shape[0]):
                constraints = cs.vertcat(constraints, A_hull[i,:] @ x[:2, -1] - b_hull[i])
        ...
        ipoptoptions = {'print_level': 0, 'print_timing_statistics': 'no', 'max_iter': 100}
        options = {'expand': True, 'print_time': False, 'ipopt': ipoptoptions}
        name = 'nmpc'
