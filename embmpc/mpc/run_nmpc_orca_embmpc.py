...
if any(arg in ('-h', '--help') for arg in sys.argv[1:]):
    parser = argparse.ArgumentParser(description='EMB-MPC with residual-driven evolving model bank')
    ...
    parser.add_argument('--n_models', type=int, default=20000, help='Number of models in the evolving bank')
    ...
    parser.add_argument('--update_interval', type=int, default=150, help='Adaptive bank update interval')
    ...
    parser.add_argument('--disable_unknown_friction_detector', action='store_true', help='Disable residual/CUSUM detector')
    parser.add_argument('--detect_warmup_steps', type=int, default=120)
    parser.add_argument('--detect_short_window', type=int, default=3)
    parser.add_argument('--detect_baseline_alpha', type=float, default=0.02)
    parser.add_argument('--detect_z_thresh', type=float, default=3.0)
    parser.add_argument('--detect_cusum_k', type=float, default=0.25)
    parser.add_argument('--detect_cusum_h', type=float, default=6.0)
    parser.add_argument('--detect_confirm_steps', type=int, default=2)
    parser.add_argument('--detect_cooldown_steps', type=int, default=80)
    parser.add_argument('--detect_protect_steps', type=int, default=45)
    parser.add_argument('--detect_aggressive_steps', type=int, default=220)
    parser.add_argument('--detect_aggressive_interval', type=int, default=25)
    parser.add_argument('--detect_aggressive_std', type=float, default=0.25)
...
from embmpc.mpc.evaluate_models_jax import make_jax_evaluator
...
parser.add_argument('--disable_unknown_friction_detector', action='store_true',
                    help='Disable residual/CUSUM based unknown friction-change detector')
parser.add_argument('--detect_warmup_steps', type=int, default=120,
                    help='Warm-up steps before enabling unknown friction-change detection')
parser.add_argument('--detect_short_window', type=int, default=3,
                    help='Short prediction-error window after detected friction change')
parser.add_argument('--detect_baseline_alpha', type=float, default=0.02,
                    help='EWMA update rate for residual baseline in unknown-change detector')
parser.add_argument('--detect_z_thresh', type=float, default=3.0,
                    help='Instantaneous z-score threshold for residual change detection')
parser.add_argument('--detect_cusum_k', type=float, default=0.25,
                    help='CUSUM drift term for residual change detection')
parser.add_argument('--detect_cusum_h', type=float, default=6.0,
                    help='CUSUM threshold for residual change detection')
parser.add_argument('--detect_confirm_steps', type=int, default=2,
                    help='Number of consecutive detector hits required to confirm a change')
parser.add_argument('--detect_cooldown_steps', type=int, default=80,
                    help='Cooldown steps after one confirmed detection')
parser.add_argument('--detect_protect_steps', type=int, default=45,
                    help='Steps to pause bank update immediately after detected change')
parser.add_argument('--detect_aggressive_steps', type=int, default=220,
                    help='Steps to use aggressive bank evolution after detected change')
parser.add_argument('--detect_aggressive_interval', type=int, default=25,
                    help='Bank update interval during detector-triggered aggressive adaptation')
parser.add_argument('--detect_aggressive_std', type=float, default=0.25,
                    help='Sampling std ratio during detector-triggered aggressive adaptation')
...
def update_model_bank(current_time_step, avg_errors_phys, current_model_idx, friction_active,
    ...
    global MODEL_BANK, MODEL_PARAMS, model_params_array, MODEL_PARAMS_ARRAY, error_windows_phys
...
UNKNOWN_FRICTION_DETECTOR_ENABLED = not args.disable_unknown_friction_detector
...
last_detect_step = -10**9
detect_protect_counter = 0
detect_aggressive_counter = 0
detected_change_steps = []
detector_z_history = []
detector_signal_history = []
detector_cusum_history = []
...
for idt in range(n_steps - horizon):
    ...
    if idt > 0:
        ...
        unknown_change_active = (detect_protect_counter > 0) or (detect_aggressive_counter > 0)
        if UNKNOWN_FRICTION_DETECTOR_ENABLED and unknown_change_active:
            effective_window = max(1, min(LookBack_W, args.detect_short_window))
        else:
        ...
        if UNKNOWN_FRICTION_DETECTOR_ENABLED:
            residual_signal = float(errors_phys[current_model_idx])
            log_signal = float(np.log(residual_signal + 1e-12))
            detector_signal_history.append(residual_signal)

            if not detector_initialized:
                detector_mean = log_signal
                detector_var = 1.0
                detector_initialized = True
                detector_z = 0.0
            else:
                detector_std = float(np.sqrt(detector_var + 1e-8))
                detector_z = float((log_signal - detector_mean) / detector_std)

                in_cooldown = (idt - last_detect_step) < args.detect_cooldown_steps
                can_detect = (idt >= args.detect_warmup_steps) and (not in_cooldown)

                if can_detect:
                    detector_cusum = max(0.0, detector_cusum + detector_z - args.detect_cusum_k)
                    detector_hit = (detector_z > args.detect_z_thresh) or (detector_cusum > args.detect_cusum_h)
                    if detector_hit:
                        detector_hit_count += 1
                    else:
                        detector_hit_count = max(0, detector_hit_count - 1)

                    if detector_hit_count >= args.detect_confirm_steps:
                        detected_event_count += 1
                        last_detect_step = idt
                        detected_change_steps.append(idt)
                        detect_protect_counter = max(detect_protect_counter, args.detect_protect_steps)
                        detect_aggressive_counter = max(detect_aggressive_counter, args.detect_aggressive_steps)
                        detector_cusum = 0.0
                        detector_hit_count = 0
                        print(f"⚡ Unknown friction change detected at iter={idt}, t={current_time:.2f}s, z={detector_z:.2f}")
                else:
                    detector_cusum = max(0.0, 0.95 * detector_cusum)
                    detector_hit_count = 0

                if (detect_protect_counter == 0) and (detect_aggressive_counter == 0) and (detector_z < 2.0):
                    diff = log_signal - detector_mean
                    detector_mean += args.detect_baseline_alpha * diff
                    detector_var = (1.0 - args.detect_baseline_alpha) * detector_var + args.detect_baseline_alpha * diff * diff

            detector_z_history.append(float(detector_z))
            detector_cusum_history.append(float(detector_cusum))
        ...
        if ENABLE_ADAPTIVE_BANK and window_count_phys >= LookBack_W and idt > LookBack_W:
            ...
            if detect_protect_counter > 0:
                detect_protect_counter -= 1
            if detect_aggressive_counter > 0:
                detect_aggressive_counter -= 1
            ...
            if detect_aggressive_counter > 0:
                effective_update_interval = args.detect_aggressive_interval
                effective_std_ratio = args.detect_aggressive_std
                aggressive = True

            if not skip_update and (idt - last_update_time) >= effective_update_interval:
                update_model_bank(idt, avg_errors_phys, current_model_idx, unknown_change_active,
...
if SAVE_PLOTS:
    ...
    plt.tight_layout()
    plt.savefig(media_dir+'/Switching_Grid_embmpc.png', dpi=400)
    ...
    plt.savefig(media_dir+'/Speeds_embmpc.png', dpi=400, bbox_inches="tight")
    ...
    plt.savefig(media_dir+'/Acc_embmpc.png', dpi=400, bbox_inches="tight")
    ...
    plt.grid(True); plt.tight_layout()
    plt.savefig(media_dir+'/steering_embmpc.png', dpi=400, bbox_inches="tight")
    ...
    plt.grid(True); plt.tight_layout()
    plt.savefig(media_dir+'/orientation_embmpc.png', dpi=400, bbox_inches="tight")
    ...
    plt.grid(True); plt.legend(); plt.tight_layout()
    plt.savefig(media_dir+'/Ds_embmpc.png', dpi=400, bbox_inches="tight")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(media_dir+'/MUs_embmpc.png', dpi=400, bbox_inches="tight")
    ...
    plt.savefig(media_dir+'/Traj_Velocity_embmpc.png', dpi=400, bbox_inches="tight")
...
results = {
    ...
    'unknown_friction_detector_enabled': bool(UNKNOWN_FRICTION_DETECTOR_ENABLED),
    'detected_event_count': int(detected_event_count),
    'detected_change_steps': [int(x) for x in detected_change_steps],
    'detected_change_times': [float(x * Ts) for x in detected_change_steps],
...
if args.output_file:
    ...
    with open(runtime_txt_file, 'w') as f:
        ...
        f.write(f"unknown_friction_detector_enabled: {UNKNOWN_FRICTION_DETECTOR_ENABLED}\n")
        f.write(f"detected_event_count: {detected_event_count}\n")
        f.write(f"detected_change_steps: {detected_change_steps}\n")
        f.write(f"detected_change_times: {[float(x * Ts) for x in detected_change_steps]}\n")
